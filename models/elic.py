...
def get_elic_models(N=192, M=320, in_channels=3):
    enc = nn.Sequential(
        conv(3, N),
...
class ELIC(CompressionModel):
    def __init__(self, in_channels=3, out_channels=3, N=192, M=320, num_slices=5, condition=0, **kwargs):
        ...
        if self.condition == 0:
            ...
            condition_channels = M
        elif self.condition == 2:
            condition_channels = 2*M
        self.ParamAggregation = nn.ModuleList(
            nn.Sequential(
                conv1x1(640 + self.groups[i+1 if i > 0 else 0] * 2 + self.groups[i + 1] * 2 + condition_channels, 640),
                nn.ReLU(inplace=True),
                conv1x1(640, 512),
                nn.ReLU(inplace=True),
    ...
    def forward(self, x, noisequant=False, condition=None):
        y = self.g_a(x)
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            if condition is not None:
                means_anchor, scales_anchor, = self.ParamAggregation[slice_index](
                    torch.concat([ctx_params_anchor_split[slice_index], support, condition], dim=1)).chunk(2, 1)
            else:
                means_anchor, scales_anchor, = self.ParamAggregation[slice_index](
            ...
            if condition is not None:
                means_non_anchor, scales_non_anchor = self.ParamAggregation[slice_index](
                    torch.concat([masked_context, support, condition], dim=1)).chunk(2, 1)
            else:
                means_non_anchor, scales_non_anchor = self.ParamAggregation[slice_index](
...
class ELIC_Encoder(ELIC):
    def __init__(self, in_channels=3, out_channels=3, N=192, M=320, num_slices=5, condition=0, **kwargs):
        super().__init__(in_channels, out_channels, N, M, num_slices, condition)
        self.g_s = None
...
class StereoELIC(nn.Module):
    def __init__(self, N=192, M=320, num_slices=5, **kwargs):
        super().__init__()
        self.common_model = ELIC(6, 3, N, M, num_slices)
        self.view_model = ELIC(6, 3, N, M, num_slices)

    def forward(self, frames):
        if not isinstance(frames, list):
            raise RuntimeError(f"Invalid number of frames: {len(frames)}.")
        x_left, x_right = frames
        common_out = self.common_model(torch.cat([x_left, x_right], dim=1))
        x_common, common_likelihoods = common_out["x_hat"], common_out["likelihoods"]
        left_out = self.view_model(torch.cat([x_left, x_common], dim=1))
        right_out = self.view_model(torch.cat([x_right, x_common], dim=1)) 
        reconstructions = [left_out["x_hat"], right_out["x_hat"]]
        frames_likelihoods = [left_out["likelihoods"], right_out["likelihoods"], common_likelihoods]
...
class StereoConditionELIC(StereoELIC):
    def __init__(self, N=192, M=320, num_slices=5, **kwargs):
        super().__init__(N, M, num_slices)
        self.view_model = ELIC(6, 3, N, M, num_slices, condition=1)
    ...
    def forward(self, frames):
        ...
        common_out = self.common_model(torch.cat([x_left, x_right], dim=1))
        x_common, common_likelihoods = common_out["x_hat"], common_out["likelihoods"]
        left_out = self.view_model(torch.cat([x_left, x_common], dim=1), condition=common_out["y_hat"])
        right_out = self.view_model(torch.cat([x_right, x_common], dim=1), condition=common_out["y_hat"]) 
        reconstructions = [left_out["x_hat"], right_out["x_hat"]]
        frames_likelihoods = [left_out["likelihoods"], right_out["likelihoods"], common_likelihoods]
...
class StereoSeperateConditionELIC(StereoConditionELIC):
    def __init__(self, N=192, M=320, num_slices=5, **kwargs):
        super().__init__(N, M, num_slices)
        self.left_model = ELIC(6, 3, N, M, num_slices, condition=2)
    ...
    def forward(self, frames):
        ...
        x_common, common_likelihoods = common_out["x_hat"], common_out["likelihoods"]
        right_out = self.view_model(torch.cat([x_right, x_common], dim=1), condition=common_out["y_hat"])
        left_out = self.left_model(torch.cat([x_left, x_common], dim=1), condition=torch.cat([common_out["y_hat"], right_out["y_hat"]],dim=1))
        reconstructions = [left_out["x_hat"], right_out["x_hat"]]
        frames_likelihoods = [left_out["likelihoods"], right_out["likelihoods"], common_likelihoods]
...
class StereoResidualConditionELIC(StereoConditionELIC):
    def __init__(self, N=192, M=320, num_slices=5, **kwargs):
        super().__init__(N, M, num_slices)
        self.view_model = ELIC(3, 3, N, M, num_slices, condition=2)
    ...
    def forward(self, frames):
        ...
        common_out = self.common_model(torch.cat([x_left, x_right], dim=1))
        x_common, common_likelihoods = common_out["x_hat"], common_out["likelihoods"]
        right_out = self.view_model(x_right-x_common, condition=torch.cat([common_out["y_hat"], common_out["y_hat"]],dim=1))
        left_out = self.view_model(x_left-x_common, condition=torch.cat([common_out["y_hat"], right_out["y_hat"]],dim=1))
        reconstructions = [left_out["x_hat"], right_out["x_hat"]]
        frames_likelihoods = [left_out["likelihoods"], right_out["likelihoods"], common_likelihoods]
...
class StereoLatentConditionELIC(StereoConditionELIC):
    def __init__(self, N=192, M=320, num_slices=5, **kwargs):
        super().__init__(N, M, num_slices)
        self.common_model = ELIC_Encoder(6, 3, N, M, num_slices)
        self.view_model = ELIC(3, 3, N, M, num_slices, condition=1)
        self.left_model = ELIC(3, 3, N, M, num_slices, condition=2)
    ...
    def forward(self, frames):
        ...
        x_left, x_right = frames
        common_out = self.common_model(torch.cat([x_left, x_right], dim=1))
        common_likelihoods = common_out["likelihoods"]
        right_out = self.view_model(x_right, condition=common_out["y_hat"])
        left_out = self.left_model(x_left, condition=torch.cat([common_out["y_hat"], right_out["y_hat"]],dim=1))
        reconstructions = [left_out["x_hat"], right_out["x_hat"]]
        frames_likelihoods = [left_out["likelihoods"], right_out["likelihoods"], common_likelihoods]
...
class StereoLatentConditionSameELIC(StereoConditionELIC):
    def __init__(self, N=192, M=320, num_slices=5, **kwargs):
        super().__init__(N, M, num_slices)
        self.common_model = ELIC_Encoder(6, 3, N, M, num_slices)
        self.view_model = ELIC(3, 3, N, M, num_slices, condition=2)
    ...
    def forward(self, frames):
        ...
        x_left, x_right = frames
        common_out = self.common_model(torch.cat([x_left, x_right], dim=1))
        ref, common_likelihoods = common_out["y_hat"], common_out["likelihoods"]
        left_out = self.view_model(x_left, condition=torch.cat([ref, ref], dim=1))
        right_out = self.view_model(x_right, condition=torch.cat([ref, right_out["y_hat"]],dim=1)) 
        reconstructions = [left_out["x_hat"], right_out["x_hat"]]
        frames_likelihoods = [left_out["likelihoods"], right_out["likelihoods"], common_likelihoods]
